...
class Db:
	...
	def add_whitelist(self, user_id: int, user_login: str) -> bool:
		print(f"{user_id=}  {user_login=}")
		query = self.cur.execute('INSERT INTO WHITELIST(user_id, user_login) VALUES(?, ?)', [user_id, user_login])
		self.commit()

	def remove_whitelist(self, id: int) -> bool:
		query = self.cur.execute('DELETE FROM WHITELIST WHERE id = ?', [id])
		self.commit()
	...
	def get_tag(self, user_id: int):
		req = self.cur.execute("SELECT tag FROM PERMISSIONS WHERE user_id = ?", [user_id])
		return req.fetchall()
